...
function Level:init()
	...
	if Game.level > 1 then
		--
		Game.stage = ((Game.level - 1) % 6 == 0) and Game.stage + 1 or Game.stage
		--
	end
	--
end
...
function Level:draw()
	...
	-- Resolves Image Stage
	LG.draw(	
				Game.assetManager:getImage("stages"),
				Game.assetManager:getImageStageQuad("stages", Game.level - ((Game.stage - 1) * 6) , Game.stage, 199, 247), 
				  6 * self.transform.scale.x,
				320 * self.transform.scale.y,
				  0,
				self.transform.scale.x,	self.transform.scale.y
			)
...
--
function Level:genereResolvegx(l,c)
	--	
	local temp = {}
	--
...
--
function Level:genereResolvegx_Col(c)
	...
	for l = 1, #self.level.resolveg.x do
		--
		self.level.resolve.x[#self.level.resolve.x+1] = self:genereResolvegx(l, c)
		--
	end	
	--
end
--
function Level:genereResolvegy(l,c)
	--	
	local temp = {}
	--
...
--
function Level:genereResolvegy_Col(c)
	...
	for l = 1, #self.level.resolveg.y do
		--
		self.level.resolve.y[#self.level.resolve.y+1] = self:genereResolvegy(l, c)
		--
	end	
	--
end
--
function Level:resolvesg_x()
	...
	for c = 1, #self.level.resolveg.x[1] do
		--
		self:genereResolvegx_Col(c)					-- Génére les solutions colonne c	
		self:resolves_x()							-- Controle les solutions colonne c	
...
function Level:resolvesg_y()
	...
	for c = 1, #self.level.resolveg.y[1] do
		--
		self:genereResolvegy_Col(c)					-- Génére les solutions colonne c	
		self:resolves_y()							-- Controle les solutions colonne c	
...
function Level:debug()
	...
	if Game.debug then
		...
		for i = 1,1 do
			...
			for k,v in ipairs(self.level.resolve.x[i]) do
				--
				LG.print(tostring(v[1])..","..tostring(v[2])..","..tostring(v[3]),10,160+((k-1)*20))
				--
			end
			--
		end		
